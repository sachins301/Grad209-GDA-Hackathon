...
//  ).toDF("Name", "Score", "Age", "Salary")
  ...
  salesCityDf.groupBy("Category", "Date").agg(sum("Revenue").as("DailyRevenue"))
    .withColumn("DailyRevenueCum",
      sum("DailyRevenue").over(Window.partitionBy("Category").orderBy("Date")))
    .show()

  // List all transactions where revenue exceeds $500.
  salesCityDf.filter($"Revenue" > 500).show()

  // Find the top 3 products with the highest revenue across all categories.
  salesCityDf.groupBy("Product").agg(sum("Revenue").as("TotalRevenue"))
    .sort(desc("TotalRevenue"))
    .limit(3).show()

  // Find the average revenue per product in each category.
  salesCityDf.groupBy("Category", "Product").agg(avg("Revenue").as("AvgRevenue")).show()

  // Count the number of transactions per customer.
  salesCityDf.groupBy("CustomerID").count()

  // Find products that have sold more than 10 units in total across all transactions.
  salesCityDf.groupBy("Product").agg(sum("Quantity").as("TotalUnits"))
    .filter($"TotalUnits" > 10).show()

  // Determine the most purchased product in each category.
  salesCityDf.groupBy("Product").agg(sum("Quantity").as("TotalUnits"))
    .sort(desc("TotalUnits"))
    .limit(1).show()

  // Write the data partitioned by Category into Parquet files
  salesCityDf.write.mode(SaveMode.Overwrite).partitionBy("Category").parquet("salesCityDf.parquet")

  // Write the data bucketed by Product into ORC format.
//  salesCityDf.write.bucketBy(3, "Product").format("orc").save("salesCityDf.orc")

  val discountData = Seq(
    ("Electronics", 10),
    ("Furniture", 5),
    ("Clothing", 15)
  ).toDF("Category", "Discount")

  // Perform a broadcast join to apply discounts on each transaction and
  // calculate the discounted revenue.
  val discountedSalesDf = salesCityDf.join(broadcast(discountData), Seq("Category"), "left_outer")
    .withColumn("DiscountedRevenue", col("Revenue") * (lit(1) - col("Discount") / 100.0))
  discountedSalesDf.show()

  // Cache the dataset after calculating revenue and use it in subsequent transformations.
  discountedSalesDf.persist(StorageLevel.MEMORY_AND_DISK)

  // Repartition the dataset based on Category and compare the execution time
  // with and without partitioning.
  val discountedSalesDfRepart = discountedSalesDf.repartition(3, $"Category")
  ...
  // Creating DataFrame with None values
  val incompleteData = spark.createDataFrame(
    spark.sparkContext.parallelize(Seq(
      Row("2023-01-01", "Electronics", null, 2, 1000, "C001"),
      Row("2023-01-02", null, "Smartphone", 5, 500, "C002"),
      Row(null, "Furniture", "Chair", 10, null, "C003")
    )),
    schema2
  )
  ...
  val filledData = incompleteData
    .withColumn("Category", when(col("Category").isNull, "Unknown").otherwise(col("Category")))
  filledData.show()

  // Drop rows with null values in the Product column.
  val cleanedData = filledData.na.drop(Seq("Product"))
  cleanedData.show()

//  //  Convert a static dataset to a streaming one
//  val streamData = discountedSalesDf.writeStream
//    .format("console")
//    .start()
//
//  // Process real-time revenue and filter transactions with revenue > $100
//  discountedSalesDf.select($"Category", $"Revenue")
//    .filter($"Revenue" > 100)
//    .writeStream
//    .outputMode("append")
//    .format("console")
